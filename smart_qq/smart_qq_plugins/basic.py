...
def callout(msg, bot):
    if "晶狗" in msg.content:
        reply = bot.reply_msg(msg, return_function=True)
        logging.info("RUNTIMELOG " + str(msg.from_uin) +
                     " calling me out, trying to reply....")
...
def repeat(msg, bot):
    ...
    if len(recorder.msg_list) > 0 and recorder.msg_list[
            -1].content == msg.content:
        if str(msg.content).strip() not in ("", " ", "[图片]", "[表情]"):
            logging.info("RUNTIMELOG " + str(msg.group_code) +
                         " repeating, trying to reply " + str(msg.content))
            reply(msg.content)
