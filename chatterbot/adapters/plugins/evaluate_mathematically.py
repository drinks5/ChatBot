...
class EvaluateMathematically(PluginAdapter):
    ...
    def load_data(self, language):
        ...
        if language == "english":
            with open(os.path.join(
                    os.path.dirname(
                        __file__), 'data', "math_words_EN.json")) as data_file:
    ...
    def substitute_words(self, string):
        ...
        for word in self.data["words"]:
            condensed_string = re.sub('_'.join(word.split(' ')),
                                      self.data["words"][word],
                                      condensed_string)

        for number in self.data["numbers"]:
            condensed_string = re.sub(
                number, str(self.data["numbers"][number]), condensed_string)

        for scale in self.data["scales"]:
            condensed_string = re.sub("_" + scale,
        ...
        for chunk_index in range(0, len(condensed_string)):
            if self.is_integer(condensed_string[chunk_index]) or self.is_float(
                    condensed_string[chunk_index]):
                i = 1
                start_index = chunk_index
                end_index = -1
                while (chunk_index + i < len(condensed_string) and
                       (self.is_integer(condensed_string[chunk_index + i]) or
                        self.is_float(condensed_string[chunk_index + i]))):
                ...
                condensed_string[start_index] = "( " + condensed_string[
                    start_index]
